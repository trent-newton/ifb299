...
<thead>
  ...
  <script>
  $(document).ready(function()class="table"
      {
          $("#myTable").tablesorter();
      }
  );
...
<?php
$sql = "SELECT teacherreviews.*,users.firstName AS studFN, users.lastName AS studLN, stuff.firstName AS teachFN, stuff.lastName AS teachLN FROM users INNER JOIN teacherreviews ON userID=studentID INNER JOIN users AS stuff ON stuff.userID= teacherreviews.teacherID WHERE reviewStatus = 'Public'";
